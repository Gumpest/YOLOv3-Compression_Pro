...
def weighted_boxes_fusion_batch(prediction, weights=None, iou_thr=0.5, thresh=0.0001):
    for image_i, pred in enumerate(prediction):
        # pred[anchor, features]
        print(pred)
        x, y, w, h = pred[:, 0], pred[:, 1], pred[:, 2], pred[:, 3]
        boxes_list = [x, y, w, h]
        scores_list = pred[:, 4] * pred[:, 5]
        labels_list = pred[:, 6]
        # boxes, scores, labels = weighted_boxes_fusion(boxes_list, scores_list, labels_list, weights=weights, iou_thr=iou_thr, thresh=thresh)
