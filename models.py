...
# update module 1、depth_wise  2、se block
# update activation 1、h_swish 2、relu6
...
def create_modules(module_defs, img_size, arc):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        # MobileNetV3
            ...
            stride = int(mdef['stride'])
            pad = (kernel_size - 1) // 2 if int(mdef['pad']) else 0
            modules.add_module('depth_wise', nn.Conv2d(in_channels=output_filters[-1],
            ...
            if bn:
                modules.add_module('BatchNorm2d', nn.BatchNorm2d(filters, momentum=0.1))
            if mdef['activation'] == 'leaky':
                modules.add_module('leaky_relu', nn.LeakyReLU(0.1, inplace=True))
            elif mdef['activation'] == 'h_swish':
            ...
            if 'filters' in mdef:
                filters = int(mdef['filters'])
                modules.add_module('se_block', SeBlock(in_channel=filters))
        ...
        output_filters.append(filters)  # 知晓上一层
...
class SeBlock(nn.Module):
    def __init__(self, in_channel, reduction=4):
        ...
        # self.Excitation.add_module('Hardsigmoid', nn.Sigmoid())
...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            mtype = mdef['type']
            # 算子注册
            if mtype in ['convolutional', 'quantize_convolutional', 'upsample', 'maxpool', 'se', 'depthwise']:
                x = module(x)
            elif mtype == 'route':
                layers = [int(x) for x in mdef['layers'].split(',')]
                if len(layers) == 1:
                    ...
                    # -1需要上采样，34不需要
                    try:
                        # print([layer_outputs[i].shape for i in layers])
